...
Pada bagian ini akan dijelaskan hal-hal mengenai \textit{data encryption standard} dimulai dari sejarah \textit{data encryption standard}, struktur \textit{data encryption standard}, dan proses enkripsi \textit{data encryption standard}.
...
Sebagai contoh, posisi ke-1 dari matriks \begin{math}mp\end{math} menunjukkan angka 58. Maka, \textit{bit} ke-58 dari masukan akan menjadi \textit{bit} ke-1 dari keluaran. Gambar \ref{fig:prosespermutasi} menunjukkan ilustrasi dari proses permutasi yang sudah dijelaskan.
...
\begin{enumerate}
	...
	\begin{equation}
		...
		\label{eq:xor}
	\end{equation}
	\item Tetapi, untuk putaran ke-16, bagian kiri dari \textit{ciphertext} (\begin{math}L_i\end{math}) diperoleh dari bagian kiri dari \textit{plaintext} (\begin{math}L_{i-1}\end{math}) yang di \textit{exclusive-or} (XOR) hasil pemetaan bagian kanan dari \textit{plaintext} (\begin{math}R_{i-1}\end{math}) dan kunci putaran \begin{math}K_i\end{math} dengan fungsi \begin{math}f\end{math}. Persamaan \ref{eq:lastround1} menunjukkan langkah yang sudah dijelaskan.
	...
	Sementara itu, bagian kanan dari ciphertext (\begin{math}R_{i}\end{math}) diperoleh dari bagian kanan dari \textit{plaintext} (\begin{math}R_{i-1}\end{math}). Persamaan \ref{eq:lastround2} menunjukkan langkah yang sudah dijelaskan.
...
Hasil akhir dari fungsi khusus adalah \textit{word} \begin{math}X\end{math} dan \textit{word} \begin{math}Y\end{math}. \textit{Word} \begin{math}X\end{math} akan menjadi \textit{word} keluaran\begin{math}A\end{math} dan \textit{word} \begin{math}Y\end{math} akan menjadi \textit{word} keluaran \begin{math}E\end{math}. Ilustrasi dari hasil keluaran ini dapat dilihat pada Gambar \ref{fig:masukankeluaranword}.
...
Sebagai contoh, diasumsikan bentuk matriks dari persamaan \ref{eq:gaussmatriks0} ditunjukkan oleh Matriks \ref{eq:gaussmatriks2}.
